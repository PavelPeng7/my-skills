...
具体的 Eagle 库路径和 API Token 存储在 agent 记忆中，执行时从 memory 获取。
...
6. **多文件夹归属** — `folders` 是数组，一个资源可属于多个文件夹。
...
def load_library(library_path):
    ...
    def map_folders(folders, parent=""):
        for f in folders:
            ...
            if f.get('children'):
                map_folders(f['children'], name)
    map_folders(meta['folders'])
    return meta, folder_map, images_dir
```

### 1. 搜索素材

```python
def search_items(images_dir, folder_map, keyword=None, ext=None, folder_path=None, limit=30):
    results = []
...
```python
def get_stats(images_dir, folder_map):
    folder_counts = {}
    type_counts = {}
    total = 0
    for d in glob.glob(os.path.join(images_dir, '*.info')):
...
```python
def get_item_detail(images_dir, item_id_or_name):
    for d in glob.glob(os.path.join(images_dir, '*.info')):
        meta_path = os.path.join(d, 'metadata.json')
        if not os.path.exists(meta_path):
            continue
...
```python
def export_inventory(images_dir, folder_map, folder_path=None):
    items = []
    for d in glob.glob(os.path.join(images_dir, '*.info')):
        meta_path = os.path.join(d, 'metadata.json')
        if not os.path.exists(meta_path):
...
Eagle 运行时提供本地 HTTP API（默认 `localhost:41595`）。API Token 从 memory 中获取。

```python
import urllib.request, json

def eagle_api(endpoint, token, data=None, base='http://localhost:41595'):
...
`/api/folder/create` 在 Eagle 4.0.0 有 bug：返回 `success` 但 `parent` 不生效。**不要用 API 建文件夹**。
...
```python
def move_to_folder(images_dir, item_id, target_folder_id, mtime_path):
    for d in glob.glob(os.path.join(images_dir, '*.info')):
        meta_path = os.path.join(d, 'metadata.json')
        if not os.path.exists(meta_path):
            continue
        ...
        if item['id'] == item_id:
            ...
            with open(mtime_path, 'r', encoding='utf-8') as fp:
                mtime = json.load(fp)
            import time
...
- **缩略图** — `images/*.info/` 目录下有 `*_thumbnail.png`，实际文件名是 `{id}{ext}`，不是 metadata 中的 `name`。
